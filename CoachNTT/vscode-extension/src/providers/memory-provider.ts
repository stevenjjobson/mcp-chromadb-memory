...
export class MemoryProvider implements vscode.TreeDataProvider<MemoryItem> {
            ...
            try {
                ...
                if (this.tierStats.working_tier) {
                    items.push(new MemoryItem(
                        'Working Tier (48h)',
                        `${this.tierStats.working_tier.count || 0} items`,
                ...
                if (this.tierStats.session_tier) {
                    items.push(new MemoryItem(
                        'Session Tier (14d)',
                        `${this.tierStats.session_tier.count || 0} items`,
                        vscode.TreeItemCollapsibleState.Collapsed,
                        'session'
                    ));
                }
                ...
                if (this.tierStats.long_term_tier) {
                    items.push(new MemoryItem(
                        'Long-term Tier',
                        `${this.tierStats.long_term_tier.count || 0} items`,
                        vscode.TreeItemCollapsibleState.Collapsed,
                        'long_term'
                    ));
                }
    ...
    private formatMemories(memories: any[]): MemoryItem[] {
    ...
    getMemoryItem(memoryId: string): any | undefined {
